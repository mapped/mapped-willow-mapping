...
foreach($solution in $(Get-Solutions)) {
    ...
    push-location $rootPath
        ...
        dotnet pack $solution --include-source --include-symbols --version-suffix $buildNumber --no-restore -p:Version=$VERTUPLE -p:AssemblyVersion=$VERTUPLE -p:FileVersion=$VERTUPLE -p:RepositoryUrl=https://github.com/$githubRepo -p:RepositoryType=git -p:RepositoryBranch=$githubBranch -p:RepositoryCommit=$githubCommit -c Release /bl:"$solution.build.binlog" /flp1:errorsOnly;logfile=$solution.Errors.log 
        if (! $?) {
